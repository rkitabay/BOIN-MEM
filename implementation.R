...
a <- 1 # alpha of a quasi-beta prior distribution 
b <- a
...
p2dose<-select_dose_BM(sim,act,mtd,monitor.ce=monitor,a,b)


ce<-enroll_Stage2(sim,p2dose,p2ch=sim$sia[iai]/sim$chsize,true.et,monitor.ce=monitor,a,b)
monitor<-ce$monitor

p2dose<-select_rp2d(sim,act,mtd,monitor.ce=monitor,a,b)
ce<-enroll_Stage2(sim,p2dose,p2ch=sim$Np2/sim$chsize+left.ch-sum(sim$sia)/sim$chsize,true.et,monitor.ce=monitor,a,b)
...
if(sum(ce$act2.dose)>1){
  ...
  for(j in 1:length(p2act)){
    ...
    if(sim$betex=="upp"||sim$betex=="none"){
      if(j==length(p2act)){
        upex<-oneside_ex(Nexdose=sim$Ndose-currj,lowside=F)
      }else upex<-oneside_ex(Nexdose=0,lowside=F)
    }else if(sim$betex=="2sd"){
      if(j==length(p2act)){
        upex<-oneside_ex(Nexdose=sim$Ndose-currj,lowside=F)
      }else upex<-oneside_ex(Nexdose=p2act[j+1]-currj-1,lowside=F)
    }else if(sim$betex=="all"){
      upex<-oneside_ex(Nexdose=sim$Ndose-currj,lowside=F)
    }
    
    if(sim$betex=="upp"||sim$betex=="2sd"){
      if(j==1){
        lwex<-oneside_ex(Nexdose=currj-1,lowside=T)
      }else lwex<-oneside_ex(Nexdose=currj-(p2act[j-1]+1),lowside=T)
    }else if(sim$betex=="none"){
      if(j==1){
        lwex<-oneside_ex(Nexdose=currj-1,lowside=T)
      }else lwex<-oneside_ex(Nexdose=0,lowside=T)
    }else if(sim$betex=="all"){
      lwex<-oneside_ex(Nexdose=currj-1,lowside=T)
    }
    ...
    comp<-c((currj-1)-Nlwex,Nlwex,1,Nupex,(sim$Ndose-currj)-Nupex)
