...
cat_utility<-function(Ndose,true.et,u11,u00){
  
  true1.util<-c()
  true2.util<-c()
  for(i in 1:Ndose){
    true1.util[i]<-u00*(1-true.et$p1tox[i])*(1-true.et$p1eff[i])+1*(1-true.et$p1tox[i])*true.et$p1eff[i]+
      0*true.et$p1tox[i]*(1-true.et$p1eff[i])+u11*true.et$p1tox[i]*true.et$p1eff[i]
    true2.util[i]<-u00*(1-true.et$p2tox[i])*(1-true.et$p2eff[i])+1*(1-true.et$p2tox[i])*true.et$p2eff[i]+
      0*true.et$p2tox[i]*(1-true.et$p2eff[i])+u11*true.et$p2tox[i]*true.et$p2eff[i]
  }
  cat("Phase1: ",true1.util,"\n")
  cat("Phase2: ",true2.util,"\n")
}

select_dose_BM<-function(Ndose,pt.target,pe.null,ctox,ceff,Narm,BOIN.ldes,act,mtd,monitor.ce,a,b){
...
select_dose_33CE<-function(act,mtd,monitor.ce,a,b){
...
enroll_Stage2_33CE<-function(Np2,s2,assoc,p2dose,p2ch,true.et,monitor.ce,a,b){ 
  nx.ce2<-nx(monitor.ce)
...
ThreePlusThree<-function(Ndose,chsize,Np1,assoc,s1,true.et,monitor){
  if(chsize!=3)stop("chsize is not 3")
  currj<-1
  act.dose<-c(rep(TRUE,Ndose),FALSE)
  cnt.dose<-rep(0,Ndose)
  add.ch<-F
